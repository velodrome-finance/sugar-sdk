...
def test_total_value_is_substantial(latest_pool_epochs):
    # Live-chain data drifts; the threshold catches a pricing/pagination regression that zeroes
    # everything out, not a precise market-conditions check. Keep it low enough to survive bad weeks.
    total = sum(e.total_fees + e.total_incentives for e in latest_pool_epochs)
    assert total > 1_000, f"expected total fees + incentives > $1k, got ${total:,.2f}"
