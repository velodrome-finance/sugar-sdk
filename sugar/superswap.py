...
# %% auto 0
__all__ = ['suppported_chains', 'SuperswapRelayer', 'HTTPSuperswapRelayer', 'AsyncSuperswap']
...
suppported_chains = ["OP", "Lisk", "Uni"]  # Example chains, extend as needed
...
class AsyncSuperswap:
    ...
    def check_chain_support(self, from_token: Token, to_token: Token) -> None:
        ...
        if from_chain.name not in suppported_chains or to_chain.name not in suppported_chains:
            raise ValueError(f"Superswap only supports {suppported_chains}. Got {from_chain.name} -> {to_chain.name}")
