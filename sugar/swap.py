...
def build_super_swap_data(i: SuperSwapDataInput) -> SuperSwapData:
    ...
    if destination_inputs:
        ...
        calls.append(ICACallData(
            ...
            value=0,
            data=Web3().eth.contract(abi=get_abi("erc20")).encode_abi("approve", args=[i.destination_router, MAX_UINT256])
        ))
        ...
        calls.append(ICACallData(
            to=to_bytes32_str(i.to_bridge_token.token_address),
            value=0,
            data=Web3().eth.contract(abi=get_abi("erc20")).encode_abi("approve", args=[i.destination_router, 0])
        ))
