__all__ = ['ADDRESS_ZERO', 'MAX_UINT256', 'MAX_UINT128', 'MAX_ABS_TICK', 'normalize_address', 'chunk', 'amount_to_k_string',
           'format_currency', 'format_percentage', 'amount_to_m_string', 'float_to_uint256', 'get_future_timestamp',
           'apply_slippage', 'price_to_tick', 'tick_to_price', 'nearest_tick', 'sqrt_ratio_x96_from_price',
...
def price_to_tick(price: float, decimals0: int, decimals1: int) -> int:
    """Price (token1 per token0) -> raw tick."""
...
def tick_to_price(tick: int, decimals0: int, decimals1: int) -> float:
    """Inverse of `price_to_tick`."""
    return 1.0001 ** tick / 10 ** (decimals1 - decimals0)

def nearest_tick(tick: int, tick_spacing: int) -> int:
    """Snap to nearest valid tick for `tick_spacing`, clamped to MAX_ABS_TICK."""
    r = round(tick / tick_spacing) * tick_spacing
    if r < -MAX_ABS_TICK: return r + tick_spacing
    if r >  MAX_ABS_TICK: return r - tick_spacing
    return r

def sqrt_ratio_x96_from_price(price: float, decimals0: int, decimals1: int) -> int:
    """Price (token1 per token0) -> Q64.96 sqrt(P). Used to initialize a brand-new CL
    pool — pass as the `sqrt_price_x96` kwarg on `chain.quote_concentrated_deposit`
    when `pool.sqrt_ratio == 0`."""
    if price <= 0: raise ValueError("price must be positive")
    return int(math.sqrt(price * 10 ** (decimals1 - decimals0)) * 2 ** 96)
