...
def pool_type_label(pool_type: int) -> str:
    """Raw `type` int → 'cl-<N>' (>0), 'stable' (=0), or 'volatile' (=-1)."""
...
class LiquidityPool:
    ...
    is_cl: bool
    # current tick (CL only; 0 for basic)
    tick: int
    # current sqrt(P) * 2**96 (CL only; 0 for basic)
    sqrt_ratio: int
    total_supply: float
    decimals: int
    token0: Token
    ...
    token1_fees: Amount
    pool_fee: float
    # gauge contract address (ADDRESS_ZERO if no gauge is configured)
    gauge: str
    # True if the gauge accepts new deposits; False after a gauge is killed
    gauge_alive: bool
    ...
    def from_tuple(
        ...
        token0_fees, token1_fees = t[24], t[25]
        ...
        return LiquidityPool(
            ...
            nfpm=normalize_address(t[29]),
            alm=normalize_address(t[30]),
        )
    ...
    def for_creation(cls, settings: ChainSettings, token0: Token, token1: Token, *,
        ...
        if tick_spacing is not None:
            ...
            return LiquidityPool(
                ...
                pool_fee=0,
                gauge=ADDRESS_ZERO, gauge_alive=False, gauge_total_supply=0,
