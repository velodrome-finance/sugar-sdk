...
# %% auto 0
__all__ = ['QUOTER_STABLE_POOL_FILLER', 'QUOTER_VOLATILE_POOL_FILLER', 'PreparedRoute', 'RouteInput', 'QuoteInput', 'Quote']
...
@dataclass
class QuoteInput:
    from_token: Token; to_token: Token; path: List[LiquidityPoolForSwap]; route: PreparedRoute; amount_in: int

    def to_tuple(self) -> tuple: return (self.from_token, self.to_token, tuple(self.path), self.route, self.amount_in, self.amount_out)

    @staticmethod
    def from_tuple(t: tuple) -> "QuoteInput":
        from_token, to_token, path, route, amount_in, amount_out = t
        return QuoteInput(from_token=from_token, to_token=to_token, path=list(path), route=route, amount_in=amount_in, amount_out=amount_out)
